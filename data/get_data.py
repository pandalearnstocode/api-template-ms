...
def _update_hash_log(logs: List[dict], log_file: pathlib.Path = base_dir / 'hash_log.csv'):
    ...
    DF_LOGS.sort_values(by='log_datetime', ascending=False).loc[:MAX_LOG_SIZE].to_csv(
...
def dvc_update(
    ...
    try:
        ...
        logger.error(
            f'Error occured during DVC tracker update!\n{repr(e)}\n{traceback.format_exc()}'
        )
        return False
...
def get_data(
    ...
    if not update_dvc and skip_dvc:
        ...
        logger.info(
            f'{len(files_downloaded)} Existing Downloads, {n_downloads} New files Downloaded\n'
        )
    else:
        ...
        if status_update is False:
            ...
            logger.info(
                f'{len(files_downloaded)} files Downloaded, {n_downloads} New files Downloaded.'
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--force', action='store_true', help='Replace all files and update dvc trackers'
    )
    ...
    parser.add_argument(
        '-c', '--auto-commit', action='store_true', help='Enable DVC files auto commit'
    )
