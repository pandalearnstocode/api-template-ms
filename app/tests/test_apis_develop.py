...
def test_api_a(api_token):
    ...
    response = client.get(
        f'{URL}/api_a/a', headers={'Accept': 'application/json', 'Authorization': api_token,},
    )
...
def test_api_b(api_token):
    ...
    # Authorized but should raise 400 error.
    response = client.get(
        f'{URL}/api_b/b', headers={'Accept': 'application/json', 'Authorization': api_token,},
    )
    assert response.status_code == HTTPStatus.UNPROCESSABLE_ENTITY

    # Successful request.
    response = client.get(
        f'{URL}/api_b/0', headers={'Accept': 'application/json', 'Authorization': api_token,},
    )
    assert response.status_code == HTTPStatus.OK
